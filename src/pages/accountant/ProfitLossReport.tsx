  ...
  useEffect(() => {
    const fetchProfitLossData = async () => {
      try {
        ...
        console.log('≡ƒôè [ProfitLossReport] Fetching P&L data from new API...');
        const response = await getProfitAndLoss({
          startDate: startDate.toISOString().split('T')[0],
          endDate: endDate.toISOString().split('T')[0]
        });

        console.log('≡ƒôè [ProfitLossReport] API Response:', response);
            ...
            <div className="bg-slate-50 rounded-2xl overflow-hidden">
              <table className="w-full">
                <tbody>
                  ...
                  <tr className="border-b border-slate-100">
                    <td className="px-6 py-3">
                      <span className="text-xs font-bold text-slate-500">Calculated as: SUM(cost_price ├ù quantity_sold)</span>
                    </td>
                    <td className="px-6 py-3 text-right">
